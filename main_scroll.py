...
def mousebuttondown_event(button):
    if button == 1:
            ...
            if g.player.main == "tool":
                if "_sword" in g.player.tool and visual.sword_swing <= 0:
                    if orb_names["purple"] in g.player.tool:
                        ...
                        visual.max_sword_swing = visual.sword_swing = 220
                        visual.sword_swing_xvel, visual.sword_swing_yvel = two_pos_to_vel(g.mouse, g.player.rrect.center)
                    visual.angle = -90
        ...
        if g.stage == "play":
            ...
            target_chunk, abs_pos = pos_to_tile(g.scaled_mouse)
...
def set_midblit(midblit, target_chunk, abs_pos):
...
def init_world(type_):
    ...
    if type_ == "new":
        ...
        if g.w.mode == "adventure":
            g.player.inventory = ["workbench", "furnace", "coal", "chicken", "slime"]
    ...
    for rel_x in range(CW):
        ...
        for rel_y in range(CH):
            ...
            if name not in empty_blocks:
                chunk_data[target_pos] = Block(name, ore_chance)
    ...
    for rel_y in range(CH):
        for rel_x in range(CW):
            ...
            if abs_pos in chunk_data:
                ...
                light_level = (block.ore_chance + 0.5) * 255
                name = block.name
                if name not in empty_blocks:
                    blit_x, blit_y = (rel_x * BS, rel_y * BS)
                    light_level = max(light_level, 0)
                    light_level = min(light_level, 255)
                    (lighting, [light_level] * 3, (blit_x, blit_y, BS, BS))
...
class World:
    ...
    def modify(self, setto, target_chunk, abs_pos, update=True):
        nbg = non_bg(setto)
        ...
        if setto in empty_blocks:
            ...
            self.data[target_chunk][abs_pos] = Block(setto)
    ...
    def trigger(self, target_chunk, abs_pos, wait=0):
        ...
        # workbench
            ...
            set_midblit(nbg, target_chunk, abs_pos)
    ...
    def __init__(self):
        ...
        self.menu_widgets = {
            ...
            "sliders": SmartList([
                ...
                Slider(win.renderer,   "Entities",      (0, 1, 10, 100), 0,                               tooltip="Maximum number of entities updateable",                             **_menu_slider_kwargs),
            ]),
        }
        _sy = _y = 190
        _x = 130
...
class Player(Scrollable, SmartVector):
    ...
    def get_cols(self, rects_only=True):
        if rects_only:
            ...
            return [data for data in self.block_data if self._rect.colliderect(data[1]) and is_hard(data[0].name)]
    ...
    def adventure_move(self):
        ...
        # other wichtige collisions with blocks
        """
        for block in all_blocks:
            if self.rect.colliderect(block.rect):
                # spike plant
...
def main(debug):
    with nullcontext() if debug else redirect_stdout(open(os.devnull, "w")):
        ...
        # strings = []
        # for i in range(20):
        #     ball = PhysicsEntity(win.renderer, win.space, (i + 1) * 40, 100 + rand(-30, 30), d=1, static=i in (0, 19))
        #     balls.append(ball)
        # for index, ball in enumerate(balls):
        #     with suppress(IndexError):
        #         string = PhysicsEntityConnector(win.renderer, win.space, ball, balls[index + 1])
        ...
        while running:
            ...
            if g.stage == "play":
                # filling
                win.renderer.draw_color = pygame.Color("lightskyblue2")
                win.renderer.draw_rect((0, 0, *win.size))
                # # day-night cycle
                    ...
                    for y in range(V_CHUNKS):
                        for x in range(H_CHUNKS):
                            ...
                            if target_chunk not in g.w.data:
                                g.w.data[target_chunk], g.w.metadata[target_chunk], g.w.terrains[target_chunk], g.w.lightings[target_chunk] = generate_chunk(target_x, target_y)
                            # init chunk render
                            ...
                            chunk_rect = pygame.Rect(chunk.topleft, (terrain_tex.width, terrain_tex.height))
                            # render chunk
                            ...
                            #     # rendering the block
                                ...
                                # img = g.w.blocks[name]
                                # win.renderer.blit(img, scroll_pos)
                                num_blocks += 1
                                ...
                                if block.broken > 0:
                                    try:
                                        win.renderer.blit(breaking_sprs[int(block.broken)], scroll_pos)
                                    except:
                ...
                # P L A Y  B L I T S -------------------------------------------------------------------------- #
                # blitting background
                win.renderer.blit(tool_holders_img, (tool_holders_x, tool_holders_y))
                win.renderer.blit(inventory_img, (inventory_x, inventory_y))
            ...
            if g.stage == "play":
                ...
                for data in chunk_texts:
                    (t1, t2), pos = data
                    write(win.renderer, "center", t1, orbit_fonts[25], BLACK, pos[0] * S, (pos[1] - 20) * S)
                    # write(win.renderer, "center", t2, orbit_fonts[18], BLACK, pos[0], pos[1] + 20)
                ...
                if g.player.main == "block" and g.player.block:
                    ...
                    write(win.renderer, "center", t | block, orbit_fonts[15], g.w.text_color, x, y + yo)

                elif g.player.main == "tool" and g.player.tool:
                    # write tool name
                    x = tool_holders_x * S + tool_holders_width * S / 2
                    tool = tshow(g.player.tool).upper()
                    write(win.renderer, "center", t | tool, orbit_fonts[inventory_font_size], g.w.text_color, x, y + yo)
                    if is_gun(g.player.tool):
                ...
                for index, block in enumerate(g.player.inventory):
                    if block is not None:
                        if g.player.inventory_amounts[index] != float("inf"):
                            write(win.renderer, "center", g.player.inventory_amounts[index], orbit_fonts[inventory_font_size], g.w.text_color, x, y + 24)
                        else:
                            write(win.renderer, "center", INF, arial_fonts[18], g.w.text_color, x, y + 27)
                    x += 33
                ...
                if g.saving_structure:
                    write(win.renderer, "midtop", "structure", orbit_fonts[9], DARK_GREEN, win.width - 30, 50)
            ...
            # post-scale home
                ...
                if g.loading_world:
                    (win.renderer, BLACK, (win.width / 2 - 100, win.height / 2 - 15, 200, 30), 3)
                    (win.renderer, LIGHT_GREEN, (win.width / 2 - 98, win.height / 2 - 13, g.loading_world_perc * 2 - 4, 26))
                    write(win.renderer, "center", f"{int(g.loading_world_perc)}%", orbit_fonts[20], BLACK, *win.center)
            ...
            tex = Texture.from_surface(win.renderer, surf)
            tex.draw(dstrect=(0, 0, 100, 50))
